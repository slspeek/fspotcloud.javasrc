...
public class TagNodeTestFactory {
	...
	public final static PhotoInfo FIRST_PICTURE_INFO = new PhotoInfo("42",
			"Me", date, 2);
	public final static PhotoInfo SECOND_PICTURE_INFO = new PhotoInfo("102",
			"You", laterDate, 1);
	...
	public TagNode getSingleNodeWithOnePicture() {
		...
		node.setCachedPhotoList(new PhotoInfoStore(
				newArrayList(FIRST_PICTURE_INFO)));
		node.setCount(1);
		return node;
	}
	...
	public TagNode getSingleNodeWithTwoPictures() {
		...
		node.setTagName("Self portraits");
		node.setDescription("Images of myself");
		node.setCachedPhotoList(new PhotoInfoStore(newArrayList(
				FIRST_PICTURE_INFO, SECOND_PICTURE_INFO)));
		node.setCount(2);
		return node;
