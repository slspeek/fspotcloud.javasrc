...
@GwtCompatible
public class DoubleImagePresenterImpl
		implements
			DoubleImageView.ImagePresenter,
			AsyncCallback<List<PhotoInfo>> {
	private final Logger log = Logger.getLogger(DoubleImagePresenterImpl.class
			.getName());
	...
	public DoubleImagePresenterImpl(DoubleImageView imageView,
			Navigator navigator, Slideshow slideshow, IScheduler scheduler) {
	...
	public void setCurrentPlace(SlideshowPlace currentPlace) {
		...
		navigator.getPageAsync(currentPlace.getTagId(),
				currentPlace.getPhotoId(), 1, this);
	}

	@Override
	public void onFailure(Throwable caught) {
		log.log(Level.WARNING, "Error during information for photo lookup",
				caught);
	}
