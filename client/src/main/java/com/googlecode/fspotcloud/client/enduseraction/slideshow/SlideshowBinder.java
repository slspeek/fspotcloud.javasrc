...
public class SlideshowBinder extends AbstractBinder {
	...
	@Inject
	public SlideshowBinder(CategoryDef categoryDef,
			FasterHandler fasterHandler, PauseHandler pauseHandler,
			SlowerHandler slowerHandler, StartHandler startHandler,
			StopHandler stopHandler, SlideshowActions actions) {
	...
	public void build() {
		...
		binding = new Relevance(FlagsRule.needing(Flags.CAN_GO_NEXT_IMAGE
				.name()), SlideshowPlace.class, BasePlace.class)
	...
	private Relevance get(int code) {
		return new Relevance(SlideshowPlace.class)
