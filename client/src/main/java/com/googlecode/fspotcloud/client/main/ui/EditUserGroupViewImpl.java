...
public class EditUserGroupViewImpl extends Composite
		implements
			EditUserGroupView {
	private final Logger log = Logger.getLogger(EditUserGroupViewImpl.class
			.getName());
	private static final EditUserGroupBinder uiBinder = GWT
			.create(EditUserGroupBinder.class);
	@UiField
	TextBox nameTextBox;
	@UiField
	...
	public EditUserGroupViewImpl(AdminButtonFactory factory,
			GroupActions groupActions, DashboardActions dashboardActions) {
	...
	interface EditUserGroupBinder
			extends
				UiBinder<Widget, EditUserGroupViewImpl> {
	}
}
