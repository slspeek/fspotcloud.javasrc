...
public class ApplicationLateBinder {
	...
	ApplicationLateBinder(ConfigBuilder configBuilder,
			...
			TreeFocusHandler treeFocusHandler, DemoFactory demoFactory,
		...
		configBuilder.bindHandler(applicationActions.hide_controls,
				hideControlsHandler);
		configBuilder.bindHandler(applicationActions.tree_focus,
				treeFocusHandler);
		configBuilder.bindHandler(applicationActions.demo,
				demoFactory.getDemo());
		configBuilder.bindHandler(applicationActions.reloadTree,
				reloadTreeHandler);
		configBuilder.bindHandler(applicationActions.goToLatest,
				goToLatestHandler);
		configBuilder.bindHandler(applicationActions.show_shortcuts,
				helpActionsFactory.getShortcutsAction());
		configBuilder.bindHandler(applicationActions.toggleAutoHide,
				toggleAutoHideHandler);
	}
}
