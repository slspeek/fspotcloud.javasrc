...
public class ApplicationBinder extends AbstractBinder {
	...
	public ApplicationBinder(AboutHandlerFactory handlerFactory,
			DashboardHandler dashboardHandler, LoginHandler loginHandler,
			LogoutHandler logoutHandler, ZoomInHandler zoomInHandler,
			ZoomOutHandler zoomOutHandler, CategoryDef categoryDef,
			HelpActionsFactory helpActionsFactory,
			UsersHelpHandler usersHelpHandler, ApplicationActions actions) {
	...
	@Override
	public void build() {
		Relevance helpBinding = new Relevance(BasePlace.class,
				SlideshowPlace.class).addDefaultKeys(new KeyStroke('H'),
				shift(KeyStroke.KEY_FORWARD_SLASH));
		bind(actions.show_help, usersHelpHandler, helpBinding);

		Relevance hideHelpBinding = new Relevance()
				.addDefaultKeys(new KeyStroke(KeyCodes.KEY_ESCAPE));
		bind(actions.hide_help, helpActionsFactory.getCloseHelp(),
				hideHelpBinding);

		Relevance aboutBinding = new Relevance(BasePlace.class, HomePlace.class)
				.addDefaultKeys(new KeyStroke('A'));
		bind(actions.about, aboutHandlerFactory.getAboutHandler(), aboutBinding);

		FlagsRule admin = new FlagsRule().needs(Flags.ADMIN.name());
		Relevance binding = new Relevance(admin, BasePlace.class)
				.addDefaultKeys(new KeyStroke('D'))
		...
		binding = new Relevance(FlagsRule.excluding(Flags.LOGGED_ON.name()),
				BasePlace.class).addDefaultKeys(KeyStroke.N);
		bind(actions.login, loginHandler, binding);
		binding = new Relevance(needing(Flags.LOGGED_ON.name()),
				BasePlace.class, LoginPlace.class, DashboardPlace.class,
		...
		binding = new Relevance(BasePlace.class, LoginPlace.class,
				DashboardPlace.class).addDefaultKeys(alt('L'));
	...
	private Relevance get(int characterCode) {
		return new Relevance(BasePlace.class).addDefaultKeys(new KeyStroke(
				characterCode));
	}
