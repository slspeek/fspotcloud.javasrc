...
public class RasterActions {
	...
	@Inject
	public RasterActions(Resources resources,
			SetRasterActionFactory setRasterActionFactory) {
		RESOURCES = resources;
		this.setRasterActionFactory = setRasterActionFactory;
		mail_fullsize = new ActionUIDef("mail-fullsize", "Mail fullsize",
		...
		toggle_tabular_view = new ActionUIDef("raster", "Toggle raster",
				"Toggle tabular viewing", RESOURCES.tabularIcon());
		set_default_raster = new ActionUIDef("reset", "Reset raster",
				"Resets raster to defaults", RESOURCES.resetIcon());
		remove_row = new ActionUIDef("remove-row", "Remove row",
				"Removes one row from the raster", RESOURCES.removeRowIcon());
		remove_column = new ActionUIDef("remove-column", "Remove column",
				"Removes one column from the raster",
				RESOURCES.removeColumnIcon());
		add_row = new ActionUIDef("add-row", "Add row",
				"Adds one row to raster", RESOURCES.addRowIcon());
		add_column = new ActionUIDef("add-column", "Add column",
				"Adds one column to raster", RESOURCES.addColumnIcon());
	}
}
