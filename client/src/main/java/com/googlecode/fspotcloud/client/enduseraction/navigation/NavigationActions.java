...
public class NavigationActions {
	...
	public NavigationActions(Resources resources) {
		RESOURCES = resources;
		end = new ActionUIDef(END_ID, "End",
				"Go to the last image of the category", RESOURCES.endIcon());
		page_down = new ActionUIDef(PAGE_DOWN_ID, "Page down",
				"Go one page forward", RESOURCES.pageDownIcon());
		row_down = new ActionUIDef(ROW_DOWN_ID, "Row down", "Go one row down",
				RESOURCES.rowDownIcon());
		next = new ActionUIDef(NEXT_ID, "Next", "Next image in this category",
				RESOURCES.nextIcon());
		back = new ActionUIDef(BACK_ID, "Back",
				"Previous image in this category", RESOURCES.backIcon());
		row_up = new ActionUIDef(ROW_UP_ID, "Row up", "Go one row back",
				RESOURCES.rowUpIcon());
		page_up = new ActionUIDef(PAGE_UP_ID, "Page up", "Go one page back",
				RESOURCES.pageUpIcon());
		home = new ActionUIDef(HOME_ID, "Home",
				"Go to the first image of the category", RESOURCES.homeIcon());
		rss_feed = new ActionUIDef("rss_feed", "RSS-feed",
				"The RSS-feed for this category", RESOURCES.rssReedIcon());
