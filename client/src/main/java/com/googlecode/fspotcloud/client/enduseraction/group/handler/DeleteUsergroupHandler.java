...
public class DeleteUsergroupHandler implements IActionHandler {
	...
	public void performAction(String actionId) {
		...
		if (info != null) {
			statusView.setStatusText("Sending request to delete group "
					+ info.getName() + " to the server");
			dispatch.execute(new DeleteGroupAction(info.getId()),
					new AsyncCallback<VoidResult>() {
						@Override
						public void onFailure(Throwable caught) {
							statusView.setStatusText("Deleting group "
									+ info.getName()
			...
			statusView
					.setStatusText("Please make a selection before attempting a delete");
		}
	}
}
