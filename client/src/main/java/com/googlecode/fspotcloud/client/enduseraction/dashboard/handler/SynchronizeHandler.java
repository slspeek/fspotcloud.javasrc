...
public class SynchronizeHandler implements IActionHandler {
	...
	public void performAction(String actionId) {
		statusView
				.setStatusText("Calling to server with a request to synchronize with the peer");
		dispatcher.execute(new UserSynchronizesPeerAction(),
				new AsyncCallback<VoidResult>() {
					public void onFailure(Throwable caught) {
						statusView
								.setStatusText("The synchronize request could not be full-filled");
					}

					@Override
					public void onSuccess(VoidResult result) {
						statusView
								.setStatusText("The synchronize request was successfully scheduled for the peer");
					}
				});
	}
}
