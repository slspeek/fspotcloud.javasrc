...
import static com.googlecode.fspotcloud.keyboardaction.KeyStroke.plain;
...
public class RasterBinder extends AbstractBinder {
	...
	public void build() {
		...
		bind(actions.set_default_raster, resetRasterHandler, new Relevance(
				BasePlace.class).addDefaultKeys(KeyStroke.shift('0')));
		Relevance binding = new Relevance(BasePlace.class).addDefaultKeys(
				plain('1'), KeyStroke.NUM_PAD_TIMES).addRule(BasePlace.class,
				excluding(Flags.TREE_FOCUS.name()), plain('1'),
				KeyStroke.SPACE, KeyStroke.NUM_PAD_TIMES);
		bind(actions.toggle_tabular_view, toggleTabularViewHandler, binding);
		bind(actions.set_raster_2x2, setRasterHandlerFactory.get(2),
				getKey('2'));
		bind(actions.set_raster_3x3, setRasterHandlerFactory.get(3),
				getKey('3'));
		bind(actions.set_raster_4x4, setRasterHandlerFactory.get(4),
				getKey('4'));
		bind(actions.set_raster_5x5, setRasterHandlerFactory.get(5),
				getKey('5'));
		bind(actions.set_raster_6x6, setRasterHandlerFactory.get(6),
				getKey('6'));
	}

	private Relevance getKey(char code) {
		return new Relevance(BasePlace.class)
				.addDefaultKeys(new KeyStroke(code));
	}
