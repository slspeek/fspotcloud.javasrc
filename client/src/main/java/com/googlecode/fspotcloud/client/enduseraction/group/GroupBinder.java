...
public class GroupBinder extends AbstractBinder {
	...
	@Inject
	public GroupBinder(CategoryDef categoryDef, GroupActions actions) {
	...
	public void build() {
		...
		binding = new Relevance(TagApprovalPlace.class).addDefaultKeys(
				KeyStroke.I, KeyStroke.INSERT);
		configBuilder.register(category, actions.grantGroup, binding);
		binding = new Relevance(TagApprovalPlace.class).addDefaultKeys(
				KeyStroke.DELETE, KeyStroke.X);
		configBuilder.register(category, actions.revokeGroup, binding);

	}

	private Relevance get(int characterCode) {
		return new Relevance(ManageGroupsPlace.class)
