...
public abstract class TreePresenterBase
		implements
			TreeView.TreePresenter,
			Provider<Cell<TagNode>> {
	...
	protected TreePresenterBase(TreeView treeView, DataManager dataManager,
	...
	protected void openSelectedTreeNode(TreeNode rootTreeNode, TagNode tagNode) {
		...
		for (Integer index : path) {
			...
			if (child == null) {
				log.log(Level.FINE, "node: " + current
						+ " has not returned child " + index);
				break;
			} else {
				current = child;
			}
	...
	protected void updatePlace() {
		if (place != null && model != null) {
			log.log(Level.FINE,
					"Update place with non-trivial place and model has been set"
							+ place);

			TagNode selectedNode = model.findByTagId(place.getTagId());
			TreeNode root = treeView.getRootNode();
			log.log(Level.FINE, "tree model root and selected node " + root
					+ " " + selectedNode);
			if (root != null) {
