...
public class RemoveTagsFromPeerHandlerTest {
	...
	public void testExecute(TaskQueueDispatch dispatchAsync, TagDao tagManager)
		...
		RemoveTagsDeletedFromPeerAction action = new RemoveTagsDeletedFromPeerAction(
		...
		verify(dispatchAsync).execute(
				new RemoveTagsDeletedFromPeerAction(
						newArrayList(new TagRemovedFromPeer("1"))));
		verify(dispatchAsync).execute(
				new RemovePhotosFromTagAction("2", newArrayList(ID_A, ID_B)));
