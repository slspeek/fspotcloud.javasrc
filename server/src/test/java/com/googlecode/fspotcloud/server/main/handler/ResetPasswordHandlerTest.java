...
public class ResetPasswordHandlerTest {
	...
	private ResetPasswordAction action = new ResetPasswordAction(RMS_FSF_ORG,
			BOO, "no_password");

	@Test
	public void testNotRegistered() throws Exception {
		ResetPasswordResult result = handler.execute(action, null);
		Assert.assertEquals(ResetPasswordResult.Code.NOT_REGISTERED,
				result.getCode());
	}
	...
	public void testNotVerified() throws Exception {
		...
		ResetPasswordResult result = handler.execute(action, null);
		Assert.assertEquals(ResetPasswordResult.Code.NOT_VERIFIED,
				result.getCode());
	}
	...
	public void testWrongCode() throws Exception {
		...
		ResetPasswordResult result = handler.execute(action, null);
		Assert.assertEquals(ResetPasswordResult.Code.WRONG_CODE,
				result.getCode());
