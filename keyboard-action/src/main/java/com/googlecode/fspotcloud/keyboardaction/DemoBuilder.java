...
public class DemoBuilder {
	...
	@Inject
	private DemoBuilder(ActionUIRegistry actionUIRegistry, EventBus eventBus,
	...
	public DemoBuilder addStep(ActionUIDef actionUIDef, int pauseMilles) {
		ActionDemoStep step = new ActionDemoStep(actionUIDef, eventBus,
				pauseMilles, getContent(actionUIDef));
		demo.getStepList().add(step);
		return this;
	}

	public DemoBuilder addStep(String actionId, int pauseMilles,
			String overridingDescription) {
		ActionUIDef actionUIDef = actionUIRegistry.getAction(actionId);
	...
	public DemoBuilder addStep(ActionUIDef actionUIDef, int pauseMilles,
			String overridingDescription) {
		SafeHtml content = new SafeHtmlBuilder().appendEscaped(
				overridingDescription).toSafeHtml();
		ActionDemoStep step = new ActionDemoStep(actionUIDef, eventBus,
				pauseMilles, content);
		demo.getStepList().add(step);
		return this;
	...
	private SafeHtml getContent(ActionUIDef actionUIDef) {
		List<KeyStroke> keyStrokes = keyboardPreferences
				.getDefaultKeysForAction(actionUIDef.getId());
		SafeHtml result = helpContentGenerator.getHelpText(actionUIDef,
				keyStrokes);
		return result;
	}
}
