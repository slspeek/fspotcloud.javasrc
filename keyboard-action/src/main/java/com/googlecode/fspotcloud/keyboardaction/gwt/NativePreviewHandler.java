...
public class NativePreviewHandler implements Event.NativePreviewHandler {
	...
	private final Logger log = Logger.getLogger(NativePreviewHandler.class
			.getName());
	...
	public void onPreviewNativeEvent(final Event.NativePreviewEvent preview) {
		...
		if (!event.getType().equalsIgnoreCase("keydown")
				|| keycode == KeyCodes.KEY_ALT || keycode == KeyCodes.KEY_SHIFT
		...
		log.log(Level.FINEST, "Event preview in keydown-event  " + keyString
				+ " keycode: " + keycode);

		final PlaceContext placeContext = placeContextProvider.get();
		List<String> actionIdList = keyboardPreferences.get(placeContext,
				new KeyStroke(modifiers, keycode));
		final String pressed = keyString + " pressed.";
		keyPressPopup.setSafeHtml(new SafeHtmlBuilder().appendEscaped(pressed)
				.toSafeHtml());
		...
		for (final String actionId : actionIdList) {
			...
			keyPressPopup.setSafeHtml(new SafeHtmlBuilder().appendEscaped(
					pressed + ", processing action: "
							+ actionUIRegistry.getAction(actionId).getName())
					.toSafeHtml());
