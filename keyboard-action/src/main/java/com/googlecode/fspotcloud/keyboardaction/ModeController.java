...
public class ModeController implements IModeController {
	...
	private PlaceContext placeContext = new PlaceContext(
			Place.NOWHERE.getClass());

	@Inject
	private ModeController(KeyboardPreferences keyboardPreferences,
			IPlaceController placeController, WidgetRegistry widgetRegistry) {
	...
	private void fireEnabledStateEvens() {
		PlaceContext newPlaceContext = new PlaceContext(placeController
				.getWhere().getClass(), getFlags());
		log.log(Level.FINEST, "firingEnabledStateEvents: old: " + placeContext
				+ " new: " + newPlaceContext);
		if (!this.placeContext.equals(newPlaceContext)) {
			this.placeContext = newPlaceContext;
			log.log(Level.FINER, "New place context " + newPlaceContext
					+ " All action buttons we be notified.");
			for (String actionId : keyboardPreferences.allActions()) {
				boolean relevant = keyboardPreferences.isRelevant(actionId,
						newPlaceContext);
				...
				if (relevant) {
					...
					List<KeyStroke> keys = keyboardPreferences
							.getKeysForAction(newPlaceContext, actionId);
