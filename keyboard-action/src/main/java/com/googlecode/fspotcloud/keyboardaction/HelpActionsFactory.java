...
public class HelpActionsFactory {
	...
	@Inject
	private HelpActionsFactory(TwoColumnHelpPopup twoColumnHelpPopup,
			SingleColumnHelpPopup singleColumnHelpPopup,
			ShortcutsPopup shortcutsPopup, HelpResources helpResources,
	...
	public IActionHandler getHelpAction(final HelpConfig helpConfig) {
		IActionHandler result = new IActionHandler() {
			...
			public void performAction(String actionId) {
				...
				firstColumn = helpContentGenerator.getHelpText(helpConfig
						.getFirstColumn());
				if (helpConfig.getSecondColumn().isEmpty()) {
					if (singleColumnHelpPopup.isShowing()) {
						...
						final SafeHtml optionalContent = helpConfig
								.getOptionalContent();
						if (optionalContent != null) {
							singleColumnHelpPopup
									.setOptionalContentDiv(optionalContent);
						}
					...
					secondColumn = helpContentGenerator.getHelpText(helpConfig
							.getSecondColumn());
	...
	public IActionHandler getShortcutsAction() {
		IActionHandler result = new IActionHandler() {
			...
			public void performAction(String actionId) {
				...
				Set<String> actions = keyboardPreferences
						.allRelevantActions(placeContext);
				SafeHtml content = helpContentGenerator.getShortcuts(actions,
						placeContext);
				...
				if (shortcutsPopup.isShowing()) {
					...
					shortcutsPopup.setPopupPosition(wWidth - width, wHeight
							- height);
				}
