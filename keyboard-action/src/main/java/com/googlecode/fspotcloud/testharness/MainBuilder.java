...
public class MainBuilder implements UIRegistrationBuilder {
	...
	public static final ActionUIDef LOGIN_DEF = new ActionUIDef(LOGIN, "Login",
			"Logon");
	public static final ActionUIDef HOME_DEF = new ActionUIDef(HOME, "Home",
			"Go home");
	public static final ActionUIDef GO_OUT_DEF = new ActionUIDef(GO_OUT,
			"Go out", "Go out place");
	public static final ActionUIDef LOGOUT_DEF = new ActionUIDef(LOGOUT,
			"Logout", "Logout");
	public static final ActionUIDef DEMO_DEF = new ActionUIDef(DEMO, "Demo",
			"Play a demo");
	...
	private ActionUIDef stopDemoDef = new ActionUIDef("quit-demo", "Quit demo",
			"Stops all demos");;
	...
	@Override
	public void build() {
		Relevance HOME_ONLY_G = new Relevance(
				needing(MainBuilder.FLAG_LOGGED_ON), HomePlace.class);
		...
		ActionCategory modeTwoSetters = configBuilder
				.createCategory("Mode 2 setters");
		ActionCategory otherModeSetters = configBuilder
				.createCategory("Other mode setters");
		ActionCategory helpCategory = configBuilder.createCategory("Help");
		configBuilder.addBinding(modeTwoSetters, LOGIN_DEF,
				new IActionHandler() {
					@Override
		...
		configBuilder.addBinding(otherModeSetters, HOME_DEF,
				new IActionHandler() {
					@Override
					public void performAction(String actionId) {
		...
		configBuilder.addBinding(modeTwoSetters, GO_OUT_DEF,
				new IActionHandler() {
					@Override
					public void performAction(String actionId) {
		...
		configBuilder.addBinding(otherModeSetters, LOGOUT_DEF,
				new IActionHandler() {
					@Override
					public void performAction(String actionId) {
		...
		ActionUIDef showHelpDef = new ActionUIDef(SINGLE_COLUMN_HELP, "Help",
				"Show a help popup.");
		ActionUIDef shortcutsDef = new ActionUIDef("shortcuts", "Shortcuts",
				"Show a shortcuts popup.");
		ActionUIDef show2cHelpDef = new ActionUIDef(TWO_COLUMN_HELP, "Help 2c",
				"Show a help 2-column popup.");
		ActionUIDef hideHelpDef = new ActionUIDef("hide-help", "Hide help",
				"Hide the help popup.");
		Relevance showHelpBinding = new Relevance(HomePlace.class)
				.addDefaultKeys(new KeyStroke(Modifiers.SHIFT, 191),
						new KeyStroke(Modifiers.NONE, 'H'));
		FlagsRule cond = new FlagsRule();
		cond.needs(FLAG_LOGGED_ON);
		showHelpBinding = showHelpBinding.addRule(OutPlace.class, cond,
				KeyStroke.H);
		Relevance show2cHelpBinding = new Relevance()
				.addDefaultKeys(new KeyStroke(Modifiers.SHIFT, 'H'));
		Relevance scBinding = new Relevance().addDefaultKeys(new KeyStroke(
				KeyStroke.KEY_FORWARD_SLASH));
		Relevance hideHelpBinding = new Relevance()
				.addDefaultKeys(new KeyStroke(Modifiers.NONE,
						KeyCodes.KEY_ESCAPE));

		configBuilder.addBinding(helpCategory, hideHelpDef,
				helpActionsFactory.getCloseHelp(), hideHelpBinding);
		HelpConfig helpConfig = new HelpConfig("Shortcuts");
		HelpConfig help2cConfig = new HelpConfig("Shortcuts in two columns");
		helpConfig.addToFirstColumn(helpCategory, modeTwoSetters,
				otherModeSetters);
		help2cConfig.addToFirstColumn(helpCategory, modeTwoSetters);
		help2cConfig.addToSecondColumn(otherModeSetters);
		configBuilder.addBinding(helpCategory, showHelpDef,
				helpActionsFactory.getHelpAction(helpConfig), showHelpBinding);
		configBuilder.addBinding(helpCategory, show2cHelpDef,
				helpActionsFactory.getHelpAction(help2cConfig),
				show2cHelpBinding);
		configBuilder.addBinding(helpCategory, stopDemoDef,
				demoBuilderFactory.getStopDemoHandler(), Q_BINDING);
		configBuilder.addBinding(helpCategory, shortcutsDef,
				helpActionsFactory.getShortcutsAction(), scBinding);
		...
		configBuilder.addBinding(helpCategory, DEMO_DEF, demoBuilder.getDemo(),
				DEMO_BINDING);
